          ...
          <div className="max-w-2xl mx-auto">
            ...
            <Reveal delay={100}>
              <div className="card p-6 sm:p-8">
                {form.status === 'success' && (
                  <div className="mb-5">
                    <FormStatusMessage status="success" successMsg="Your distributor enquiry has been received. We will contact you soon." />
                  </div>
                )}
                <form onSubmit={submit} noValidate>
                  <FormContainer>
                    <FormField label="Business Name" name="businessName" value={form.values.businessName} onChange={(v) => form.setValue('businessName', v)} error={form.errors.businessName} required />
                    <div className="grid sm:grid-cols-2 gap-4">
                      <FormField label="Contact Person" name="contactPerson" value={form.values.contactPerson} onChange={(v) => form.setValue('contactPerson', v)} error={form.errors.contactPerson} required />
                      <FormField label="Phone" name="phone" type="tel" value={form.values.phone} onChange={(v) => form.setValue('phone', v)} error={form.errors.phone} required />
                    </div>
                    <div className="grid sm:grid-cols-2 gap-4">
                      ...
                      <FormField label="Location / Region" name="location" value={form.values.location} onChange={(v) => form.setValue('location', v)} error={form.errors.location} required />
                    </div>
                    <FormField label="Current Business Type" name="currentBusiness" value={form.values.currentBusiness} onChange={(v) => form.setValue('currentBusiness', v)} placeholder="e.g. Retailer, Wholesaler, Distributor" />
                  ...
                  <div className="mt-6 flex flex-col sm:flex-row gap-3">
                    ...
                    <Link to="/business" className="btn-ghost">Back to Business Hub</Link>
                  </div>
                </form>
              </div>
            </Reveal>
